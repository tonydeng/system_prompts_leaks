...
获取**实时**能力数据——上下文窗口、最大输出 token、功能支持（思考、视觉、effort、结构化输出等）——请查询 Models API，而非依赖下方的缓存表格。当用户询问"X 的上下文窗口是多少"、"模型 X 是否支持视觉/思考/effort"、"哪些模型支持功能 Y"，或想在运行时按能力选择模型时使用此方法。
...
# capabilities 是无类型嵌套字典——方括号访问，在叶子节点检查 ["supported"]
caps = m.capabilities
...
# 跨所有模型筛选——直接迭代页面对象（自动分页）；不要使用 .data
[m for m in client.models.list()
 if m.capabilities["thinking"]["types"]["adaptive"]["supported"]
 and m.max_input_tokens >= 200_000]
```

顶层字段（`id`、`display_name`、`max_input_tokens`、`max_tokens`）是类型化属性。`capabilities` 是字典——使用方括号访问，而非属性访问。API 为每个模型返回完整的能力树，每个叶子节点都有 `supported: true/false`，因此方括号链式访问无需 `.get()` 守卫即可安全使用。TypeScript SDK：方法名相同，迭代时同样自动分页。
...
| 友好名称          | 别名（使用此别名）    | 完整 ID                       | 上下文窗口     | 最大输出    | 状态 |
|-------------------|---------------------|-------------------------------|----------------|------------|--------|
...
### 模型描述
- **Claude Fable 5** — Anthropic 已广泛发布的最强大模型，用于最苛刻的推理和长程代理工作。与 Opus 4.7/4.8 相同的 API 接口，但有一个新的破坏性变更：显式 `thinking: {type: "disabled"}` 会返回 400——改为省略 `thinking` 参数（思考始终开启；原始思维链不会返回——通过 `display: "summarized"` 获取摘要）。与 Opus 4.8 相同的分词器（token 计数与 Opus 4.7/4.8 大致相同）。安全分类器可能返回 `stop_reason: "refusal"`。不支持助手预填充。需要 30 天数据留存（ZDR 下不可用）。$10/$50 每百万 token；1M 上下文窗口（默认），128K 最大输出。参见 `shared/model-migration.md` → 迁移到 Claude Fable 5。
- **Claude Mythos 5** — 与 Claude Fable 5 具有相同的能力、定价、限制和 API 行为；仅模型 ID 不同。仅通过 Project Glasswing 提供，它加入（并继任了）仅限邀请的 Claude Mythos Preview（`claude-mythos-preview`）。仅在组织参与 Project Glasswing 时使用；否则使用 claude-fable-5。
- **Claude Opus 4.8** — 最强大的 Opus 级模型——高度自主，在长程代理工作、知识工作和记忆方面处于最前沿；写作更清晰、更温暖。与 Opus 4.7 相同的 API 接口（仅自适应思考；采样参数和 `budget_tokens` 已移除）。1M 上下文窗口，标准 API 定价（无长上下文溢价）。参见 `shared/model-migration.md` → 迁移到 Opus 4.8——从 4.7 到 4.8 只是模型 ID 替换加提示词重新调优，没有新的破坏性变更。
- **Claude Opus 4.7** — 上一代 Opus。高度自主；在长程代理工作、知识工作、视觉和记忆方面表现强劲。仅自适应思考；采样参数和 `budget_tokens` 已移除。1M 上下文窗口。参见 `shared/model-migration.md` → 迁移到 Opus 4.7。
- **Claude Opus 4.6** — 较旧的 Opus。支持自适应思考（推荐），128K 最大输出 token（大输出需要流式传输）。1M 上下文窗口。
- **Claude Sonnet 5** — Sonnet 级别中速度与智能的最佳组合；在编码和代理工作上接近 Opus 质量。默认开启自适应思考（省略 `thinking` 即运行自适应）；手动 `budget_tokens` 已移除；非默认采样参数被拒绝。`effort` 支持 `low`/`medium`/`high`/`xhigh`/`max`。新分词器（相同文本比 Sonnet 4.6 约多 30% token）。高分辨率视觉（2576px）。1M 上下文窗口，128K 最大输出。参见 `shared/model-migration.md` → 迁移到 Claude Sonnet 5。
- **Claude Sonnet 4.6** — 上一代 Sonnet。支持自适应思考（推荐）。1M 上下文窗口。128K 最大输出 token。
- **Claude Haiku 4.5** — 用于简单任务的最快、最具性价比的模型。
