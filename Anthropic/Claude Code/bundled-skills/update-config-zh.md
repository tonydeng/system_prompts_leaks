> **说明**：本文件为英文原文（`update-config.md`）的中文译注版，辅助理解。英文原文为权威来源，任何冲突以原文为准。占位符（如 `{model_name}`）保持原样不译。
...
如果用户希望某事在响应某个事件时自动发生，需要在 settings.json 中配置 **hook**。Memory/偏好设置无法触发自动化操作。

**以下情况需要 hook：**
- "压缩前，问我要保留什么" → PreCompact hook
- "写入文件后，运行 prettier" → 带 Write|Edit 匹配器的 PostToolUse hook
- "运行 bash 命令时，记录它们" → 带 Bash 匹配器的 PreToolUse hook
...
**修改前务必先读取现有 settings 文件。**将新设置与现有设置合并，切勿替换整个文件。

## 关键：遇到歧义时使用 AskUserQuestion

当用户请求存在歧义时，使用 AskUserQuestion 进行澄清：
- 修改哪个 settings 文件（user/project/local）
- 是添加到现有数组还是替换
- 存在多个选项时的具体值
...
**以下情况直接编辑 settings.json：**
- Hook（PreToolUse、PostToolUse 等）
...
3. **谨慎合并** - 保留现有设置，尤其是数组
4. **编辑文件** - 使用 Edit 工具（如文件不存在，请用户先创建）
5. **确认** - 告诉用户修改了什么

## 合并数组（重要！）

向权限数组或 hook 数组添加内容时，**与现有内容合并**，不要替换：
...
| `.claude/settings.json` | 项目 | 提交 | 团队级 hook、权限、插件 |
| `.claude/settings.local.json` | 项目 | Gitignore | 本项目的个人覆盖设置 |

Settings 加载顺序：user → project → local（后者覆盖前者）。
...
- `cleanupPeriodDays`：自动清理前保留对话记录的天数（默认：30；最小值 1）
...
- `spinnerTipsOverride`：覆盖 spinner 提示（`{ "excludeDefault": true, "tips": ["Custom tip"] }`）
- `syntaxHighlightingDisabled`：禁用 diff 高亮


## Hooks 配置

Hook 在 Claude Code 生命周期的特定节点运行命令。
...
**字段：**
- `systemMessage` - 向用户显示消息（所有 hook）
- `continue` - 设为 `false` 以阻止/停止（默认：true）
- `stopReason` - `continue` 为 false 时显示的消息
- `suppressOutput` - 从转录中隐藏 stdout（默认：false）
- `decision` - 对 PostToolUse/Stop/UserPromptSubmit hook 为 "block"（PreToolUse 已弃用，改用 hookSpecificOutput.permissionDecision）
- `reason` - 决策说明
...
给定事件、匹配器、目标文件和期望行为，按以下流程操作。每一步捕获不同的失败类型——一个静默无效的 hook 比没有 hook 更糟糕。

1. **去重检查。**读取目标文件。如果同一 event+matcher 上已存在 hook，显示现有命令并询问：保留、替换还是并行添加。

2. **为本项目构造命令——不要假设。**Hook 通过 stdin 接收 JSON。构造一个命令来：
   - 安全提取所需 payload——使用 `jq -r` 读入引号变量或 `{ read -r f; ... "$f"; }`，不要用不加引号的 `| xargs`（会按空格分割）
   - 按本项目运行方式调用底层工具（npx/bunx/yarn/pnpm？Makefile target？全局安装？）
   - 跳过工具无法处理的输入（格式化器通常有 `--ignore-unknown`；如没有，按扩展名守护）
   - 暂时保持 RAW——不加 `|| true`，不抑制 stderr。管道测试通过后再包装。

3. **管道测试原始命令。**合成 hook 将接收的 stdin payload 并直接管道传入：
   - `Pre|PostToolUse` on `Write|Edit`：`echo '{"tool_name":"Edit","tool_input":{"file_path":"<a real file from this repo>"}}' | <cmd>`
   - `Pre|PostToolUse` on `Bash`：`echo '{"tool_name":"Bash","tool_input":{"command":"ls"}}' | <cmd>`
   - `Stop`/`UserPromptSubmit`/`SessionStart`：大多数命令不读 stdin，所以 `echo '{}' | <cmd>` 就够了

   检查退出码和副作用（文件是否真的格式化了，测试是否真的运行了）。如果失败会得到真实错误——修复（包管理器错了？工具没装？jq 路径不对？）然后重测。成功后用 `2>/dev/null || true` 包装（除非用户想要阻塞式检查）。

4. **写入 JSON。**合并到目标文件（schema 形状见上文"Hook 结构"部分）。如果这是首次创建 `.claude/settings.local.json`，将其加入 .gitignore——Write 工具不会自动 gitignore。

5. **一次性验证语法 + schema：**

   `jq -e '.hooks.<event>[] | select(.matcher == "<matcher>") | .hooks[] | select(.type == "command") | .command' <target-file>`

   退出 0 + 打印你的命令 = 正确。退出 4 = 匹配器不匹配。退出 5 = JSON 格式错误或嵌套错误。损坏的 settings.json 会静默禁用该文件的所有设置——也要修复任何已有的格式错误。

6. **证明 hook 会触发**——仅适用于你能在本回合触发的匹配器上的 `Pre|PostToolUse`（通过 Edit 触发 `Write|Edit`，通过 Bash 触发 `Bash`）。`Stop`/`UserPromptSubmit`/`SessionStart` 在本回合之外触发——跳到第 7 步。

   对于 `PostToolUse`/`Write|Edit` 上的**格式化器**：通过 Edit 引入一个可检测的违规（两个连续空行、错误缩进、缺失分号——该格式化器会纠正的内容；不是尾随空格，Edit 在写入前会去除），重新读取，确认 hook **修复了**它。对于**其他情况**：临时在 settings.json 的命令前加 `echo "$(date) hook fired" >> /tmp/claude-hook-check.txt; `，触发匹配工具（`Write|Edit` 用 Edit，`Bash` 用无害的 `true`），读取哨兵文件。

   **务必清理**——无论验证通过与否，都还原违规、移除哨兵前缀。

   **如果验证失败但管道测试和 `jq -e` 都通过**：settings watcher 没有监视 `.claude/`——它只监视会话启动时已有 settings 文件的目录。hook 写得没错。告诉用户打开一次 `/hooks`（会重新加载配置）或重启——你自己做不到；`/hooks` 是用户 UI 菜单，打开它会结束本回合。

7. **交接。**告诉用户 hook 已生效（或按 watcher 注意事项需要 `/hooks`/重启）。引导他们到 `/hooks` 以后查看、编辑或禁用。UI 只有在 hook 出错或过慢时才显示"Ran N hooks"——静默成功按设计是不可见的。
