> **说明**：本文件为英文原文（`google-calendar.md`）的中文译注版，辅助理解。英文原文为权威来源，任何冲突以原文为准。占位符（如 `{model_name}`）保持原样不译。
...
返回用户有权访问的日历（其日历列表）。使用此工具将日历识别数据（例如"我的家庭日历"）解析为对应的 `calendar_id`（电子邮件标识符）。
...
返回指定日历上匹配所有指定约束的事件。除非用户要求，否则不应指定时间约束。对于主日历上的开放式关键词或基于主题的搜索，必须改用 search_events 工具。
